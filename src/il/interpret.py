...
class IlMachine(object):
    ...
    def inst(self, addr, inst):
        ...
        # control flow
            ...
            #print(instruction)
            bool_value = self.value(inst.a) #boolean value 
            if(bool_value):
                return Address(addr.fn, inst.result[0], 0)
            else:
                return Address(addr.fn, inst.result[1], 0)
            
            #print(addr.fn)
            #print(addr)
            #print(inst.a)
            #print(inst.b)
            #print(inst.result) #if register has 1, jump to block inst.result[0] else jump to inst.result[1]
            # TODO: IMPLEMENT

        # instrinsic
        elif inst.op == il.OPS['PRINT']:
            if( type( self.value(inst.a) ) == bool ):
                if(self.value(inst.a) == True):
                    print("true")
                else:
                    print("false")
            else:
