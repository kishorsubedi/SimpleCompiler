...
class _Parser(object):
    ...
    def if_stmt(self, idx):
        ...
        if not self.matches(block_idx, TOKEN_IDS["ELSE"]):
            n = Node("if").addkid(condition).addkid(block)
            return n, block_idx
        ...
        else_block, else_block_idx = self.dispatch(else_idx, lookahead)
        n = (Node("if")
        .addkid(condition).addkid(block).addkid(else_block))
        return n, else_block_idx
    ...
    def break_stmt(self, idx):
        _, break_idx = self.consume(idx, TOKEN_IDS["BREAK"])
        if not self.matches(break_idx, TOKEN_IDS["NAME"]):
            n = Node("break")
            return n, break_idx
    ...
    def continue_stmt(self, idx):
        _, continue_idx = self.consume(idx, TOKEN_IDS["CONTINUE"])
        if not self.matches(continue_idx, TOKEN_IDS["NAME"]):
            n = Node("continue")
